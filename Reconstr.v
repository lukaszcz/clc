...
Inductive ReconstrEmptyT : Set := Empty : ReconstrEmptyT.
...
Ltac simp0 f H :=
  ...
  let tp := type of H in
  lazymatch tp with
    | (exists x, _) => elim H; clear H; sintro tt; sintro tt
    | ?A = ?A => clear H
    | ?A -> ?A => clear H
    | ?A -> ?B = ?B => clear H
...
Ltac dsolve :=
  match goal with
    | [ |- ?G ] => notProp G; auto with nocore yhints; try solve [ repeat constructor ]
    | _ => auto with nocore yhints; try yeasy
  end.
...
Ltac sintuition :=
  simp_hyps; intuition (auto with nocore yhints); try yeasy; try subst; mcongr tt;
...
Ltac docrush :=
  yintuition; cbn in *; simp_hyps; forward_reasoning 2; yisolve; try yelles 1;
